...
class mainProg(cmd.Cmd):
    ...
    def do_pervision(self, arg):
        '\n{logs} - Afficher le fichier log.\n\n{fichiers} - Afficher le fichier CSV.\n'
        ...
        if arg == "logs":
            ...
            print("Argument erroné, faites <help pervision> pour plus d'informations.\n")
...
if __name__ == '__main__':
    if len(sys.argv) == 2: # On verifie si il n'y a que deux argument, l'application et le chemin du répertoire
        if os.path.exists(sys.argv[1]): # On verifie que le réperoire existe
            if os.path.isdir(sys.argv[1]): # On verifie si nous avons bien un répertoire
                mainProg().cmdloop()
            else:
